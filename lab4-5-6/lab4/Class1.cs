...
public class Style
{
    ...
    public Style(string color, string style)
    {
        if (color.Length > 10)
        {
            throw new InvalidStyleDescriptionException("Lenght of color can't exceed 10", color);
        }
        else if (style.Length > 15)
        {
            throw new InvalidStyleDescriptionException("Lenght of style can't exceed 15", style);
        }
        else
        {
            this.color = color;
...
public sealed class Button : Control
{
    ...
    public Button(double x, double y, double h, double w, string color, string style, string text) : base(x,y,h,w,color,style)
    {
        if(text.Length > 10)
        {
            throw new InvalidButtonNameException("Длина названия не может превышать 10 символов", text);
        }
        else this.text = text;
    }
    public override string Show()
...
public class Menu: Control
{
    ...
    {
        this.items = items;
        Debug.Assert(level < 0, "Menu level less than zero.");
        this.level = level;
    }
...
public class Window : Rectangle
{
    ...
    public Window(double x, double y, double h, double w, string title, WindowsStyle style) :base(x,y,h,w)
    {
        if(title.Length > 15)
        {
            throw new InvalidWindowNameException("Lenght of window title can't exceed 15", title);
        }
